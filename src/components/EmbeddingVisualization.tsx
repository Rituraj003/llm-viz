...
const Tooltip: React.FC<{
  ...
  return (
    ...
    >
      <strong>Cluster {hoveredPoint.cluster}</strong>
...
const Legend: React.FC<{
  ...
  return (
    <>
      <h4 style={{ marginBottom: "15px", marginTop: "5px", color: "#E0E0E0" }}>Clusters</h4>
        ...
        {[...clusterColors.entries()]
          ...
          .map(([cluster, color]) => (
            ...
            >
              ...
              />
              <span style={{ fontSize: "14px" }}>Cluster {cluster}</span>
            </div>
          ))}
      </div>
    </>
...
const EmbeddingVisualization: React.FC = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch("/gsm8k_embeddings_2d.json")
      .then((res) => {
      ...
      .then((d: DataPoint[]) => {
        setData(d);
        setActiveClusters(new Set(d.map(p => p.cluster)));
        setLoading(false);
      })
      .catch((err) => {
  ...
  const { xScale, yScale, colorScale, clusterColors, filteredData } = useMemo(() => {
    ...
    return { xScale, yScale, colorScale, clusterColors, filteredData };
  }, [data, activeClusters, dimensions]);

  // --- Canvas Drawing ---
  const drawPoints = useCallback((currentTransform: d3.ZoomTransform) => {
  ...
  useEffect(() => {
    ...
    // Axes
    const xAxis = d3.axisBottom(xScale.copy().domain(transform.rescaleX(xScale).domain()));
    const yAxis = d3.axisLeft(yScale.copy().domain(transform.rescaleY(yScale).domain()));
  ...
  const handleResetView = () => {
    ...
    canvas.transition().duration(750).call(zoom.transform as any, d3.zoomIdentity);
    svg.transition().duration(750).call(zoom.transform as any, d3.zoomIdentity);
  };

  // --- Render ---
  if (loading) return <div className="status-message">Loading embeddings...</div>;
